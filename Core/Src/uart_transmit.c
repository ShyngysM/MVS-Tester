...
//   HAL_UART_Transmit(huart, (uint8_t *)uart_buf, uart_buf_len, 100); // 100 is a timeout in ms or s?
// }
...
//    HAL_UART_Transmit(&huart3, (uint8_t *)msg, strlen(msg)); // No need to transmit the null terminator
// }
...
void uart_transmit_array(uint16_t *arr, size_t length, UART_HandleTypeDef *huart) {
    ...
    for (size_t i = 0; i < length; i++) {
        int bytes_written = snprintf(uart_buf + pos, sizeof(uart_buf) - pos, "%d ", arr[i]);
